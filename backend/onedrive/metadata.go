...
func (m *Metadata) orderPermissions(xs []*api.PermissionsType) {
	...
	hasUserIdentity := func(identity *api.IdentitySet) bool {
		...
		return identity.User.DisplayName != "" || identity.User.Email != "" || identity.User.LoginName != ""
	}
	...
	// Put Permissions with a user first, sorting by ID otherwise
	slices.SortFunc(xs, func(a, b *api.PermissionsType) int {
		aHasUser := hasUser(a)
		bHasUser := hasUser(b)
